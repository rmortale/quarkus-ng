...
```bash
docker build -f src/main/docker/Dockerfile -t kafka-minio:1.0-SNAPSHOT .
```

Once the application is published, you can run it directly from the container:

```bash
docker run -it kafka-minio:1.0-SNAPSHOT
```
...
```bash
./kafka-minio-1.0-SNAPSHOT-runner
```
...
```bash
docker build -f src/main/docker/Dockerfile.native -t native-kafka-minio:1.0-SNAPSHOT .
```

Once the application is published, you can run it directly from the container:

```bash
docker run -it native-kafka-minio:1.0-SNAPSHOT
```
